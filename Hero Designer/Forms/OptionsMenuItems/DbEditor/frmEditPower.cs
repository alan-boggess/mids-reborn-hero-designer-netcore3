...
namespace Hero_Designer
{
    public partial class frmEditPower : Form
    {
        ...
        void btnFullPaste_Click(object sender, EventArgs e)
        {
            ...
            {
                MessageBox.Show("No power data on the clipboard!", @"Unable to Paste");
            }
            else
            {
        ...
        void btnMutexAdd_Click(object sender, EventArgs e)
        {
            ...
            InputBoxResult result = InputBox.Show("Please enter a new group name.It must be different to all the others", "Add Mutex Group", "New_Group", new InputBoxValidatingHandler(inputBox_Validating));
            if (result.OK)
            {
            ...
            {
                MessageBox.Show($@"'{b}' is not unique!", @"Unable to add");
            }
            else
            {
        ...
        void btnOK_Click(object sender, EventArgs e)
        {
            ...
            {
                MessageBox.Show("Power name '" + power.FullName + " is invalid.", @"No Can Do");
            }
            else if (!PowerFullNameIsUnique(Convert.ToString(power.PowerIndex)))
            {
                MessageBox.Show("Power name '" + power.FullName + " already exists, please enter a unique name.", @"No Can Do");
            }
            else
            {
        ...
        void lblStaticIndex_Click(object sender, EventArgs e)
        {
            ...
            InputBoxResult result = InputBox.Show("Insert new static index for this power", "", Conversions.ToString(myPower.StaticIndex), new InputBoxValidatingHandler(inputBox_Validating));
            ...
            try
            {
                ...
                if (num1 < 0)
                {
                    MessageBox.Show("The static index cannot be a negative number.", "Static Index", MessageBoxButtons.OK, MessageBoxIcon.Exclamation);
                }
                else
                {
        ...
        {
            if (MessageBox.Show("If this power is required to be present, click 'Yes'.\r\nIf this power must NOT be present, click 'No'.", "Query", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.No)
            {
