...
namespace Hero_Designer
{
    public partial class frmEditArchetype : Form
    {
        ...
        void btnOK_Click(object sender, EventArgs e)
        {
            ...
            {
                MessageBox.Show("An archetype class must have at least one valid origin!", @"Oops.");
            }
            else if (cbPriGroup.Text == "" | cbSecGroup.Text == "")
            {
                MessageBox.Show(@"You must set a Primary and Secondary Powerset Group!", @"Oops.");
            }
            else
            {
                ...
                {
                    num3 = 1f;
                    MessageBox.Show(@"Hit Point value of < 1 is invalid. Hit Points set to 1");
                }
        ...
        bool CheckClassName()
        {
            ...
            for (int index = 0; index <= num1; ++index)
            {
                if (index == MyAT.Idx || !string.Equals(DatabaseAPI.Database.Classes[index].ClassName, txtClassName.Text,
                    ...
                    continue;
                MessageBox.Show(txtClassName.Text + " is already in use, please select a unique class name.", "Name in Use", MessageBoxButtons.OK, MessageBoxIcon.Information);
                return false;
            }
            return true;
        }
